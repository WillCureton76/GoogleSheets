  ...
  try {
    ...
    switch (action) {
      case "get":
        ...
        return g("GET", `/spreadsheets/${encodeURIComponent(spreadsheetId)}/values/${encodeURIComponent(range)}${url.search || ""}`);
      ...
      case "update": {
        if (req.method !== "PUT" && req.method !== "POST")
          return res.status(405).json({
            ...
            message: "Use POST or PUT for update",
          });
        const body = await readJson(req, res);
        if (!body) return;
        return g("PUT", `/spreadsheets/${encodeURIComponent(spreadsheetId)}/values/${encodeURIComponent(range)}?valueInputOption=RAW`, body);
      }

      case "append": {
        const body = await readJson(req, res);
        if (!body) return;
        return g("POST", `/spreadsheets/${encodeURIComponent(spreadsheetId)}/values/${encodeURIComponent(range)}:append?valueInputOption=RAW`, body);
      }
