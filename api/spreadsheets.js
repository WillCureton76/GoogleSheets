export default async function handler(req, res) {
  ...
  try {
    ...
    switch (action) {
      case "get": {
        if (!spreadsheetId) return res.status(400).json({ ok: false, source: "proxy", status: 400, code: "INVALID_PARAMS", message: "Require spreadsheetId" });
        return g("GET", `/spreadsheets/${encodeURIComponent(spreadsheetId)}${url.search || ""}`);
      }

      case "create": {
        if (req.method !== "POST") return res.status(405).json({ ok: false, source: "proxy", status: 405, code: "METHOD_NOT_ALLOWED", message: "Use POST for /api/spreadsheets/create" });
      ...
      case "copyto": {
        ...
        if (!spreadsheetId || !sheetId) return res.status(400).json({ ok: false, source: "proxy", status: 400, code: "INVALID_PARAMS", message: "Require spreadsheetId and sheetId" });
        const body = await readJson(req, res); if (!body) return;
      ...
      case "developermetadata/get": {
        ...
        if (!metadataId) return res.status(400).json({ ok: false, source: "proxy", status: 400, code: "INVALID_PARAMS", message: "Require metadataId" });
      ...
      default: {
        ...
        return res.status(404).json({ ok: false, source: "proxy", status: 404, code: "UNKNOWN_SPREADSHEETS_ACTION", message: `Unknown /api/spreadsheets action '${action || "(none)"}'` });
      }
    }
  } catch (err) {
